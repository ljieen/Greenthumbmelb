...
class ewdupcpDashboard {
	...
	public function __construct() {
		...
		if ( ! empty( $_POST['ewd_upcp_reset_settings'] ) ) {
		...
		<div id="ewd-upcp-dashboard-content-area">
								...
								<?php
									if ( empty( $catalogs ) ) {echo "<tr><td colspan='3'>" . __("No catalogs to display yet. Create a catalog for it to be displayed here.", 'ultimate-product-catalogue') . "</td></tr>";}
										...
										foreach ( $catalogs as $catalog ) { ?>
											<tr>
												<td><a href='post.php?post=<?php echo esc_url( $catalog->ID ); ?>&action=edit'><?php echo esc_html( $catalog->post_title ); ?></a></td>
			...
			<div id="ewd-upcp-dashboard-content-right">
				...
				<?php if ( ! $permission or get_option("EWD_UPCP_Trial_Happening") == "Yes" ) { ?>
					<div class="ewd-upcp-dashboard-new-widget-box ewd-widget-box-full" id="ewd-upcp-dashboard-get-premium-widget-box">
						...
						<div class="ewd-upcp-dashboard-new-widget-box-bottom">
							...
							<a href="https://www.etoilewebdesign.com/license-payment/?Selected=UPCP&Quantity=1" class="ewd-upcp-dashboard-get-premium-widget-button" target="_blank">UPGRADE NOW</a>
				...
				<?php
					...
					if ( empty( $products ) or ! empty( $_GET['ewd_upcp_show_reset_settings'] ) ) { ?>

						<div class="ewd-upcp-dashboard-new-widget-box ewd-widget-box-full">

							<div class="ewd-upcp-dashboard-new-widget-box-top">Settings Reset</div>

							<div class="ewd-upcp-dashboard-new-widget-box-bottom">

								<form method='post'>
									<input class="ewd-upcp-dashboard-get-premium-widget-button" type='submit' name='ewd_upcp_reset_settings' value='Reset Settings' />
								</form>

								<p>If you're trying to re-run the product import process from version 4.4.x to version 5.0.x, reset your settings using the button above, then deactivate and re-activate the plugin.</p>
	...
	public function reset_settings() {
		...
		if ( ! current_user_can( $ewd_upcp_controller->settings->get_setting( 'access-role' ) ) ) { return; }
